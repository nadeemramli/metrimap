import {
  useBulkOperations,
  type BulkUpdateData,
} from '@/features/canvas/hooks/useBulkOperations';
...
export default function BulkOperationsToolbar({
  ...
  const handleBulkUpdate = async () => {
    try {
      ...
      announce(`Error updating ${updateType}: ${String(error)}`);
    }
  };
  ...
  const handleBulkDelete = async () => {
    try {
      ...
      announce(`Error deleting items: ${String(error)}`);
    }
  };
  ...
  const handleDuplicate = async () => {
    try {
      ...
      else announce(`Failed to duplicate items: ${result.errors.join(', ')}`);
    } catch (error) {
      announce(`Error duplicating items: ${String(error)}`);
    }
  };
  ...
  const handleAddTags = async () => {
    ...
    try {
      ...
      announce(`Error adding tags: ${String(error)}`);
    }
  };
  ...
  const handleExport = async (format: 'json' | 'csv') => {
    try {
      ...
      announce(`Error exporting: ${String(error)}`);
    }
  };
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <Input
                    id="tags"
                    value={
                      Array.isArray((updateData as any).tags)
                        ? ((updateData as any).tags as string[]).join(', ')
                        : ''
                    }
                    onChange={(e) =>
                      setUpdateData((prev) => ({
              ...
              <div className="space-y-4">
                <div>
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="Compositional">
                        Compositional
                      </SelectItem>
                ...
                <div>
                  ...
                  <Input
                    ...
                    max="1"
                    step="0.1"
                    value={(updateData as any).weight || ''}
                    onChange={(e) =>
                      setUpdateData((prev) => ({
                        ...prev,

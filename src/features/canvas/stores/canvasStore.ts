...
export const useCanvasStore = create<CanvasStoreState>()(
  subscribeWithSelector((set, get) => ({
    ...
    createNode: async (
      ...
      // Check if canvas is a valid UUID (database-backed project) or a temporary "new" canvas
      const uuidRegex =
      ...
      if (!isValidProject) {
        ...
        try {
          ...
          const projectData = {
            name: state.canvas.name || 'Untitled Project',
          ...
          const newProject = await createProjectInSupabase(projectData, client);
          ...
          // Update the canvas with the real project ID
          set((state) => ({
            canvas: state.canvas
              ? {
                  ...state.canvas,
          ...
          // Now create the node in the database
          ...
          if (typeof window !== 'undefined') {
            window.history.replaceState({}, '', `/canvas/${newProject.id}`);
          }
          ...
          try {
            ...
            // Add to projects store without triggering the API again
            projectsStore.projects = [...projectsStore.projects, canvasProject];
          } catch (syncError) {
            console.warn('Failed to sync with projects store:', syncError);
          }
        } catch (error) {
          console.error('❌ Failed to promote canvas or create node:', error);
              ...
              ? {
                  ...
                  nodes: [
                    ...state.canvas.nodes,
                    {
                      ...(nodeData as any),
                      id: tempId,
                      owner: useAppStore.getState().user?.id || 'anonymous',

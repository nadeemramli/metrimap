...
export function PersistenceStatus({ className }: PersistenceStatusProps) {
  ...
  const getStatus = () => {
    if (error) return { type: 'error', message: 'Save failed', icon: AlertCircle };
    if (isSaving) return { type: 'saving', message: 'Saving...', icon: Clock };
    if (pendingChanges.size > 0) return { type: 'pending', message: `${pendingChanges.size} unsaved`, icon: WifiOff };
  ...
  return (
    <div className={cn('flex items-center gap-2 text-xs', className)}>
      <Badge variant={getVariant()} className="flex items-center gap-1.5">
        <Icon className={cn(
          'h-3 w-3',
          status.type === 'saving' && 'animate-spin'
        )} />
        {status.message}
      </Badge>

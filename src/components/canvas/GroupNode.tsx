...
export default function GroupNode({ id, data, selected }: NodeProps) {
  ...
  useEffect(() => {
    ...
    const padding = 50;
    const newSize = {
      width: childNodes.width + padding * 2,
      height: childNodes.height + padding * 2,
    };

    onResizeGroup(id, newSize);
  }, [selected, group.nodeIds, onResizeGroup, id]);
  ...
  return (
    <>
      {/* Node Resizer for resizing the group */}
      <NodeResizer
        minWidth={200}
        minHeight={120}
        isVisible={selected}
        ...
        onResize={(_, params) => {
          console.log("Resizing group:", id, params);
          onResizeGroup?.(id, { width: params.width, height: params.height });
        }}
      />
      ...
      <div
        ...
        style={{
          ...
          // Ensure proper layering within React Flow canvas
          position: "relative",
          overflow: "visible",
          // Make group background more transparent
        ...
        onDrop={(e) => {
          e.preventDefault();
          e.stopPropagation();
          // Handle node drop into group
          console.log("Node dropped into group:", id);
        }}
        onDragOver={(e) => {
          e.preventDefault();
          e.stopPropagation();
        }}
        // Let React Flow handle all interactions naturally
        // No custom event handlers to avoid conflicts
      >
        {/* Group Header */}
        ...
        {/* Resize area indicators - show when selected */}
        {selected && (
          <>
            {/* Corner resize handles */}
            <div className="absolute top-0 left-0 w-3 h-3 cursor-nw-resize pointer-events-none" />
            <div className="absolute top-0 right-0 w-3 h-3 cursor-ne-resize pointer-events-none" />
            <div className="absolute bottom-0 left-0 w-3 h-3 cursor-sw-resize pointer-events-none" />
            <div className="absolute bottom-0 right-0 w-3 h-3 cursor-se-resize pointer-events-none" />

            {/* Edge resize areas */}
            <div className="absolute top-0 left-3 right-3 h-1 cursor-n-resize pointer-events-none" />
            <div className="absolute bottom-0 left-3 right-3 h-1 cursor-s-resize pointer-events-none" />
            <div className="absolute left-0 top-3 bottom-3 w-1 cursor-w-resize pointer-events-none" />
            <div className="absolute right-0 top-3 bottom-3 w-1 cursor-e-resize pointer-events-none" />
          </>
        )}
      </div>

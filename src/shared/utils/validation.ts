...
// Enhanced with Zod integration for type-safe validation
...
export interface ZodValidationResult<T> {
  ...
  error?: string; // Single error message for simple cases
}

// Helper to convert Zod validation to legacy format
export function adaptZodValidation<T>(zodResult: {
  success: boolean;
  data?: T;
  errors?: z.ZodError["issues"];
}): ValidationResult {
  ...
  const firstError = zodResult.errors?.[0];
  return {
    isValid: false,
    error: firstError
...
export function validateMetricTitle(title: string): ValidationResult {
  ...
  if (title.trim().length < 2) {
    return {
      isValid: false,
      error: "Metric title must be at least 2 characters long",
    };
  }
  ...
  if (title.length > 200) {
    return {
      isValid: false,
      error: "Metric title must be less than 200 characters",
    };
  }
